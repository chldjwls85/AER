...
// Parameterized V4 lightweight SPARSE/ROW AER readout.
module aer_top_v4 #(
    ...
    initial begin
        if ((SENSOR_ROWS < 8) || (SENSOR_COLS < 8) ||
            ...
            $display("AER_V4_BAD_SENSOR_SIZE rows=%0d cols=%0d",
                     SENSOR_ROWS, SENSOR_COLS);
            $fatal(1);
        end
        if ((BANK_ROWS > 16) || (BANK_COLS > 16)) begin
            $display("AER_V4_BANK_ARRAY_TOO_LARGE rows=%0d cols=%0d",
                     BANK_ROWS, BANK_COLS);
            $fatal(1);
        end
        if (TILE_COUNT != (SENSOR_ROWS/2)*(SENSOR_COLS/2)) begin
            $display("AER_V4_TILE_COUNT_MISMATCH");
            $fatal(1);
        end
    end
endmodule

module aer_top_v4_128 (
    input  wire           clk,
    input  wire           rst_n,
