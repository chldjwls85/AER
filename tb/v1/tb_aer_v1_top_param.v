`timescale 1ns/1ps

module tb_aer_v1_top_param;

    localparam integer SENSOR_ROWS = 24;
    localparam integer SENSOR_COLS = 40;
    localparam integer TILE_COUNT = (SENSOR_ROWS/2) * (SENSOR_COLS/2);
    localparam integer TEST_BANK = 14;
    localparam integer TEST_TILE = 6;
    localparam integer TEST_INDEX = TEST_BANK * 16 + TEST_TILE;

    reg                    clk;
    reg                    rst_n;
    reg  [TILE_COUNT-1:0]  tile_in_valid;
    reg  [TILE_COUNT*4-1:0] tile_on_flat;
    reg  [TILE_COUNT*4-1:0] tile_off_flat;
    wire [TILE_COUNT-1:0]  tile_in_ready;
    wire [15:0]            out_data;
    wire                   out_valid;
    reg                    out_ready;
    wire                   out_last;

    reg [15:0] captured [0:2];
    reg [2:0]  captured_last;
    integer    transfer_count;
    integer    timeout;

    aer_v1_top #(
        .SENSOR_ROWS(SENSOR_ROWS),
        .SENSOR_COLS(SENSOR_COLS),
        .REGION_BANK_ROWS(2),
        .REGION_BANK_COLS(2)
    ) dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .tile_in_valid (tile_in_valid),
        .tile_on_flat  (tile_on_flat),
        .tile_off_flat (tile_off_flat),
        .tile_in_ready (tile_in_ready),
        .out_data      (out_data),
        .out_valid     (out_valid),
        .out_ready     (out_ready),
        .out_last      (out_last)
    );

    always #5 clk = ~clk;

    always @(posedge clk) begin
        if (!rst_n) begin
            transfer_count = 0;
        end else if (out_valid && out_ready) begin
            captured[transfer_count] = out_data;
            captured_last[transfer_count] = out_last;
            transfer_count = transfer_count + 1;
        end
    end

    initial begin
        clk = 1'b0;
        rst_n = 1'b0;
        tile_in_valid = {TILE_COUNT{1'b0}};
        tile_on_flat = {(TILE_COUNT*4){1'b0}};
        tile_off_flat = {(TILE_COUNT*4){1'b0}};
        out_ready = 1'b1;

        repeat (3) @(posedge clk);
        @(negedge clk);
        rst_n = 1'b1;
        if (tile_in_ready[TEST_INDEX] !== 1'b1) begin
            $display("AER_V1_PARAM_READY_FAIL");
            $fatal(1);
        end

        tile_on_flat[TEST_INDEX*4 +: 4] = 4'b1111;
        tile_in_valid[TEST_INDEX] = 1'b1;
        @(posedge clk);
        @(negedge clk);
        tile_in_valid = {TILE_COUNT{1'b0}};
        tile_on_flat = {(TILE_COUNT*4){1'b0}};

        timeout = 0;
        while ((transfer_count < 3) && (timeout < 300)) begin
            @(posedge clk);
            timeout = timeout + 1;
        end

        if (transfer_count != 3) begin
            $display("AER_V1_PARAM_TIMEOUT transfers=%0d", transfer_count);
            $fatal(1);
        end
        if ((captured[0] !== 16'hC394) ||
            (captured[2] !== 16'h8100) ||
            (captured_last !== 3'b100)) begin
            $display("AER_V1_PARAM_FAIL words=%h,%h,%h last=%b",
                captured[0], captured[1], captured[2], captured_last);
            $fatal(1);
        end

        $display("AER_V1_TOP_PARAM_PASS sensor=24x40 banks=3x5 levels=3");
        $finish;
    end

endmodule
