...
module tb_aer_bank_sparse_lossy;
    ...
    task wait_for_count;
        ...
        begin
            ...
            if (captured_count != expected_count) begin
                $display("AER_BANK_SPARSE_LOSSY_TIMEOUT expected=%0d got=%0d pending=%h",
    ...
    initial begin
        ...
        // One ON event at tile 0, pixel 2.  External receive time allows the
        // complete lossless transaction to fit in one SPARSE address word.
        send_bank(16'h0001, 64'h0000_0000_0000_0004, 64'b0);
        wait_for_count(1);
        if ((captured_word[0] !== 16'h0285) || !captured_last[0]) begin
            $display("AER_BANK_SPARSE_WORD_FAIL word=%h last=%b",
                captured_word[0], captured_last[0]);
            $fatal(1);
        end

        // The dense hotspot still selects the five-word mixed lossy packet.
        // Its payload must remain bit-identical after bounded sequential pack.
        ...
        if ((captured_word[1] !== 16'h8177) ||
            ...
            $display("AER_BANK_SPARSE_LOSSY_DENSE_FAIL words=%h,%h,%h,%h,%h",
                captured_word[1], captured_word[2], captured_word[3],
                captured_word[4], captured_word[5]);
            $fatal(1);
        end

        $display("AER_BANK_SPARSE_LOSSY_PASS sparse_words=1 dense_words=5");
        $finish;
    end
