# Full Genus synthesis driver for aer_top_v5_128.
# Synthesis stages, reports, library/PVT/effort, and I/O constraints match the
# existing V4 flow. Server-specific setup is supplied through environment hooks.

set SCRIPT_DIR [file dirname [file normalize [info script]]]
set ROOT_DIR [file normalize [file join $SCRIPT_DIR ".." ".."]]
set TOP aer_top_v5_128
set FILELIST [file join $ROOT_DIR "rtl" "filelist_v5.f"]
...
set RUN_DIR [require_env AER_GENUS_RUN_DIR]
if {[file dirname $RUN_DIR] ne "/tmp" ||
    ![string match "*_aer_genus_v5_*" [file tail $RUN_DIR]]} {
    error "V5 Genus run directory must be a unique direct child of /tmp: $RUN_DIR"
}
if {![file isdirectory $RUN_DIR]} {
    error "V5 Genus run directory does not exist: $RUN_DIR"
}

set LOG_DIR [file join $RUN_DIR "logs"]
set REPORT_DIR [file join $RUN_DIR "reports"]
set NETLIST_DIR [file join $RUN_DIR "out"]
...
puts "AER_GENUS_V5_TOP $TOP"
puts "AER_GENUS_V5_RUN_DIR $RUN_DIR"
puts "AER_GENUS_SERVER_SETUP $SERVER_SETUP_TCL"
puts "AER_GENUS_V3_IO_CONSTRAINTS $V3_IO_TCL"

# Must contain the same V4/V3 library path, slow_vdd1v0 PVT, effort, and options.
...
# Reuse the exact V4 clock/uncertainty SDC for a fair comparison.
read_sdc $CLOCK_SDC
source $V3_IO_TCL
...
set summary_path [file join $RUN_DIR "genus_v5_summary.txt"]
set summary [open $summary_path w]
puts $summary "AER V5 Genus full synthesis"
...
puts $summary "netlist=[file join $NETLIST_DIR aer_top_v5_128_mapped.v]"
puts $summary "sdc=[file join $NETLIST_DIR aer_top_v5_128_mapped.sdc]"
puts $summary "AER_GENUS_V5_FULL_SYNTHESIS_DONE"
close $summary

puts "AER_GENUS_V5_FULL_SYNTHESIS_DONE summary=$summary_path"
flush stdout
